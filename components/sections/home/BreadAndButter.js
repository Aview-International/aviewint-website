import FullWidthGradient from '../../layout/FullWidthGradient';

const BreadAndButter = () => {
  return (
    <section className="section">
      <FullWidthGradient>
        <h2 className="title mx-auto mb-4 max-w-[640px] text-center">
          Translating Content, It&apos;s Our Bread and Butter
        </h2>
        <p className="body mx-auto max-w-[1243px] text-center">
          We connect creators with world-class translators to help them build a
          global audience. The value we create speaks for itself. With over
          500M+ views, we&apos;ve helped creators and media companies meet their
          revenue and audience growth goals.
        </p>
      </FullWidthGradient>
    </section>
  );
};

export default BreadAndButter;
